My time as a STaaS technical consultant has taught me the importance of ine particular thing, it is that downtime is crtical and a ineffable factor that most organisations realise yet do not put it in words to the fullest extent possible.
Sentinel is my attempt to understand how modern monitoring systems are built by creating one from scratch.


- What Problem does sentinel solve?
    Production incidents generate huge amounts of logs.

    Finding the right log quickly is difficult.

    The longer it takes to investigate,
    the longer production remains impacted.

<<<<<<< HEAD
    My honest vision for sentinel?
=======
- My honest vision for sentinel? 
>>>>>>> c29bee9 (build: intialize backend structure and development environment)
    A Catalyst for better record keeping and analysis of logs so that man hours can be conserved during incidents and production.
  
- Who is the user?
   Backend engineer, Platform engineer, SRE, Developer basically any engineer working on production environments that constantly need moniotring.

- What should Version 0.1 do?

    Receive logs
    
    ↓
    
    Store them
    
    ↓
    
    Search them
    
    ↓
    
    View them
